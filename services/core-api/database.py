...
def init_db():
    ...
    conn.commit()
    conn.close()
    logger.info(f"Database initialized and upgraded at {DB_PATH}")
...
def save_message(session_id: str, role: str, content: str, metadata: str = None):
    """Lưu tin nhắn vào database kèm metadata (trích dẫn)."""
    try:
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute(
...
def get_session_history(session_id: str, limit: int = 50):
    """Lấy lịch sử tin nhắn của một session kèm trích dẫn."""
    try:
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute(
            "SELECT role, content, metadata FROM chat_messages WHERE session_id = ? ORDER BY timestamp ASC LIMIT ?",
            (session_id, limit)
        )
        rows = cursor.fetchall()
        conn.close()
        return [{"role": row["role"], "content": row["content"], "metadata": row["metadata"]} for row in rows]
    except Exception as e:

import os
import torch
from sentence_transformers import SentenceTransformer
from pymilvus import connections, Collection
import logging

# Blackwell Optimizations
torch.backends.cuda.matmul.allow_tf32 = True
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:True"

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("test_search")

def test_query(query_text, top_k=3):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    logger.info(f"🚀 Loading model on {device}...")
    
    # Load model with BF16 for Blackwell
    model = SentenceTransformer("BAAI/bge-m3", device=device)
    if device == "cuda":
        if torch.cuda.is_bf16_supported():
            model = model.to(dtype=torch.bfloat16)
            logger.info("✅ BF16 enabled")
        else:
            model = model.half()
            logger.info("✅ FP16 enabled")
    
    # Connect to Milvus (using localhost since we are running on host)
    MILVUS_HOST = os.getenv("MILVUS_HOST", "localhost")
    connections.connect("default", host=MILVUS_HOST, port="19530")
    
    from pymilvus import utility
    COLLECTION_NAME = "vi_legal_rag"
    if not utility.has_collection(COLLECTION_NAME):
        logger.error(f"❌ Collection {COLLECTION_NAME} does not exist!")
        return
        
    collection = Collection(COLLECTION_NAME)
    collection.load()
    
    # Encode query
    logger.info(f"🔍 Querying: '{query_text}'")
    query_vector = model.encode([query_text], normalize_embeddings=True).tolist()
    
    # Search
    search_params = {"metric_type": "L2", "params": {"nprobe": 10}}
    results = collection.search(
        data=query_vector,
        anns_field="vector",
        param=search_params,
        limit=top_k,
        output_fields=["title", "text"]
    )
    
    print("\n" + "="*80)
    print(f"KẾT QUẢ TRUY VẤN CHO: '{query_text}'")
    print("="*80)
    
    for i, hit in enumerate(results[0]):
        print(f"\n[{i+1}] Score: {hit.score:.4f}")
        print(f"Tiêu đề: {hit.entity.get('title')}")
        content = hit.entity.get('text')
        # Show first 500 characters
        print(f"Nội dung: {content[:500]}...")
        print("-" * 40)

if __name__ == "__main__":
    import sys
    query = sys.argv[1] if len(sys.argv) > 1 else "Quy định về thời giờ làm việc, thời giờ nghỉ ngơi"
    test_query(query)
